      ...
      with st.form("f_new_contract"):
        ...
        with col_c2:
          start_d = st.date_input(
              "Data de Início da Vigência", value=datetime.date.today()
          )
          end_d = st.date_input(
        ...
        if st.form_submit_button("💾 Salvar Novo Contrato e Empresa"):
          if not new_ctr_num or new_ctr_num in contracts_list:
            ...
            # 1. Salva em COMPANY exatamente 2 colunas: [cnpj, name]
            if is_new_company:
              ws_company.append_row([c_cnpj.strip(), c_nome.strip()])

            # 2. Salva em CONTRACT exatamente 6 colunas: [contract_number, company_cnpj, manager_name, start_date, end_date, regime_pagamento]
