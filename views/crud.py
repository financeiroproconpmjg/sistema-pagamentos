...
def render_crud():
    ...
    with t_create:
        ...
        if selected_contract_opt == "[ + Criar Novo Contrato ]":
            st.info(
                "💡 Selecione uma empresa cadastrada ou crie uma nova para associar ao contrato."
            )
            ...
            with st.form("f_new_contract"):
                ...
                with col_c2:
                    start_d = st.date_input("Data de Início da Vigência", value=datetime.date.today())
                    end_d = st.date_input("Data de Fim da Vigência", value=datetime.date.today() + datetime.timedelta(days=365))
                ...
                if st.form_submit_button("💾 Salvar Novo Contrato e Empresa"):
                    if not new_ctr_num or new_ctr_num in contracts_list:
                        ...
                        # 1. Se for nova empresa, salva em COMPANY (cnpj, name)
                        if is_new_company:
                            ws_company.append_row([c_cnpj, c_nome])

                        # 2. Salva em CONTRACT (contract_number, company_cnpj, manager_name, start_date, end_date, regime_pagamento)
                        ...
                        st.success(
                            f"Contrato **{new_ctr_num}** cadastrado com sucesso para a empresa **{c_nome}**!"
                        )
                        refresh_caches()
                        st.rerun()
            st.stop()
            ...
            with st.expander("📝 Formulário: Novo Empenho Global", expanded=True):
                with st.form("f_new_eg"):
                    ...
                    if st.form_submit_button("Salvar Novo Empenho Global"):
                        ...
                        st.success(
                            f"Empenho Global **#{eg_id_input}** criado para o contrato {selected_contract}!"
                        )
                        refresh_caches()
                        st.rerun()
            st.stop()
        ...
        if selected_sub_opt == "[ + Criar Novo Sub-Empenho ]":
            ...
            with st.expander("📝 Formulário: Novo Sub-Empenho", expanded=True):
                with st.form("f_new_sub"):
                    ...
                    if st.form_submit_button("Salvar Novo Sub-Empenho"):
                        existing_sub_ids = (
                            df_sub["id"].astype(str).tolist() if not df_sub.empty and "id" in df_sub.columns else []
                        )
            ...
            st.caption(
                f"📅 **Mês de Ref:** {sub_info.get('reference_month', '')} | **Valor:** R$ {sub_info.get('value', 0)}"
            )
