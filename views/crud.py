...
def render_crud():
  ...
  with t_create:
    ...
    if selected_company_opt == "[ + Cadastrar Nova Empresa ]":
      ...
      st.caption(
          f"🏢 **Empresa Selecionada:** {selected_comp_nome} | **CNPJ:**"
          f" {selected_cnpj}"
      )
    ...
    selected_contract_opt = st.selectbox(
        f"Selecione um Contrato da empresa {selected_comp_nome} ou Crie um"
        " Novo",
    ...
    if not df_eg_contract.empty:
      for _, eg_row in df_eg_contract.iterrows():
        ...
        label = (
            f"Empenho Nº {eg_num_val} (ID: {eg_id_val}) - R$"
            f" {eg_val_num:,.2f}"
        )
    ...
    if selected_eg_label == "[ + Criar Novo Empenho Global ]":
        ...
        with st.form("f_new_eg"):
          ...
          if st.form_submit_button("Salvar Novo Empenho Global"):
            if not eg_number:
              ...
              st.success(
                  f"Empenho Global **Nº {eg_number}** (ID #{next_eg_id}) criado"
                  " com sucesso!"
              )
              refresh_caches()
              st.rerun()
      ...
      st.caption(
          f"💰 **Número:** {eg_info.get('number', '---')} | **Valor Teto:** R$"
          f" {clean_num_val(eg_info.get('value', 0)):,.2f} | **Status:**"
          f" {'Cancelado' if str(eg_info.get('is_canceled')).upper() == 'VERDADEIRO' else 'Ativo'}"
      )
    ...
    if selected_sub_opt == "[ + Criar Novo Sub-Empenho ]":
      ...
      st.caption(
          ...
          f" R$ {clean_num_val(sub_info.get('value', 0)):,.2f}"
      )
